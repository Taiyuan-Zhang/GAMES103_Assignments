...
public class neo_Hookean : MonoBehaviour
{
	float dt 			= 0.002f;
    float mass 			= 1;
    ...
    void _Update()
    {
    	...
    	for(int tet=0; tet<tet_number; tet++)
    	{
            ...
            float J = A[0,0]*A[1,1]*A[2,2];
            float IIIc = J*J;
            diag[0, 0] = stiffness_0*(-1.0f/3.0f*Mathf.Pow(IIIc, -4.0f/3.0f)*2.0f*IIIc/A[0,0]*Ic+2*A[0,0]*Mathf.Pow(IIIc, -1.0f/3.0f))+2*stiffness_1*(J-1)*A[1,1]*A[2,2];
            diag[1, 1] = stiffness_0*(-1.0f/3.0f*Mathf.Pow(IIIc, -4.0f/3.0f)*2.0f*IIIc/A[1,1]*Ic+2*A[1,1]*Mathf.Pow(IIIc, -1.0f/3.0f))+2*stiffness_1*(J-1)*A[0,0]*A[2,2];
            diag[2, 2] = stiffness_0*(-1.0f/3.0f*Mathf.Pow(IIIc, -4.0f/3.0f)*2.0f*IIIc/A[2,2]*Ic+2*A[2,2]*Mathf.Pow(IIIc, -1.0f/3.0f))+2*stiffness_1*(J-1)*A[0,0]*A[1,1];
