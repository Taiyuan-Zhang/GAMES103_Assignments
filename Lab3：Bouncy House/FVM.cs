...
public class FVM : MonoBehaviour
{
    ...
    void _Update()
    {
    	...
    	for(int tet=0; tet<tet_number; tet++)
    	{
    		...
    		//Green Strain
			Matrix4x4 F_T = F.transpose;
			Matrix4x4 G = Num_MUL_M(0.5f, M_SUB_M(F_T*F, Matrix4x4.identity));

    		//Second PK Stress
			Matrix4x4 S = M_ADD_M(Num_MUL_M(2.0f*stiffness_1, G),Num_MUL_M(stiffness_0*(G[0,0]+G[1,1]+G[2,2]), Matrix4x4.identity));
			Matrix4x4 P = F*S;
    		//Elastic Force
