...
public class Rigid_Bunny_by_Shape_Matching : MonoBehaviour
{
		...
		for(int i=0; i<X.Length; i++){
			if(Dot_Product(X[i]-P, N)<0.04f){
				if(Dot_Product(V[i], N)<0){
					...
					V[i] = v_N_new + v_T_new;
					X[i] = X[i] + Mathf.Abs(Dot_Product(X[i]-P, N))*N*1.5f;
					restitution *= 0.5f;
				}
			}
		}
    ...
    void Update()
    {
		...
		if(launched==true){
			...
			Matrix4x4 A_tmp = Matrix4x4.zero;
			for(int i=0; i<X.Length; i++){
				Vector3 tmp = X[i] - c;
				A_tmp[0, 0] += tmp[0]*Q[i][0];
				A_tmp[0, 1] += tmp[0]*Q[i][1];
				A_tmp[0, 2] += tmp[0]*Q[i][2];
				A_tmp[1, 0] += tmp[1]*Q[i][0];
				A_tmp[1, 1] += tmp[1]*Q[i][1];
				A_tmp[1, 2] += tmp[1]*Q[i][2];
				A_tmp[2, 0] += tmp[2]*Q[i][0];
				A_tmp[2, 1] += tmp[2]*Q[i][1];
				A_tmp[2, 2] += tmp[2]*Q[i][2];
			}
			A_tmp[3, 3] = 1;
			Matrix4x4 A = A_tmp * QQt.inverse;
