...
public class PBD_model: MonoBehaviour {
	...
	void Strain_Limiting()
	{
		...
		for(int idx=0; idx<E.Length; idx+=2){
			...
			Vector3 x_j = vertices[j];
			Vector3 x_ij = x_i - x_j;
			float x_ij_norm = x_ij.magnitude;
			sum_x[i] += 0.5f*(x_i+x_j+L[idx/2]*x_ij/x_ij_norm);
			sum_n[i] ++;
			sum_x[j] += 0.5f*(x_i+x_j-L[idx/2]*x_ij/x_ij_norm);
			sum_n[j] ++;
		}
		for(int i=0; i<vertices.Length; i++){
	...
	void Collision_Handling()
	{
		...
		for(int i=0; i<X.Length; i++){
			if(i==0 || i==20) continue;
			Vector3 d = X[i] - c;
			float d_norm = d.magnitude;
			if(d_norm < radius){
				V[i] += 1/t*(c+radius*d/d_norm-X[i]);
				X[i] = c + radius*d/d_norm;
			}
		}
		mesh.vertices = X;
	}
